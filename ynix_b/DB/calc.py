...
class Attacker(object):
    def __init__(self, class_id, ap, aap, acc, acc_rate, crit_rate, monster_ap, kama_damage, demi_damage, human_damage, other_damage, crit_damage, back_damage, down_damage, air_damage, ap_combat_buffs, crit_combat_buffs, ap_debuffs, acc_combat_buffs, acc_debuffs, human_damage_debuffs):
...
# --------------------------------------------------------------------------
    def calc_mean(self):
        ...
        hit_damage_mean = (e_ap_mean * hit_value + base_damage_mean
                           if e_ap_mean > 0
                           else base_damage_mean)*.8
        # -------------------------------------
...
# ------------------------------------------------------------------------------------------------
    def calc_range(self):
        ...
        base_damage_high = (self.t_ap + species_ap_high) * \
            (hit_value/self.attacker.basic)
        # ------------------------------
        hit_damage_low = (e_ap_low * hit_value + base_damage_low
                          if e_ap_low > 0
                          else base_damage_low
                          )*.8

        hit_damage_high = (e_ap_high * hit_value + base_damage_high
                           if e_ap_high > 0
                           else base_damage_high
                           )*.8
        #------------------------------
        #crit range = 
        crit_range_low = hit_damage_low + hit_damage_low*(1 + self.attacker.crit_damage)
        crit_range_high = hit_damage_high + hit_damage_high*(1 + self.attacker.crit_damage)
        #------------------------------
        return [round(hit_damage_low), round(hit_damage_high), round(crit_range_low), round(crit_range_high)]
# --------------------------------------------------------------------------------------------

    def calc_if_crit(self, hit_in, this_hit):
        crit_chance = (self.attacker.crit_rate + self.attacker.crit_combat_buffs + hit_in['pvp_crit_rate']
                       if self.defender.class_id != 100
                       else self.attacker.crit_rate + self.attacker.crit_combat_buffs + hit_in['pve_crit_rate'])
        roll = random.randrange(0, 100)

        return (this_hit + this_hit*(1 + self.attacker.crit_damage)
